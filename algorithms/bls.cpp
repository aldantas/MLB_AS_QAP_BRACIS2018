...

{
	...
	/******************** main BLS loop ********************/
	for (current_iteration = 0; ; current_iteration++)
	{
		...
		if((clock() - start)/static_cast<double>(CLOCKS_PER_SEC) >= timeout)
			break;
	}
	best_time -= start;
...
int main(int argc, char *argv[])
{
	...
	BLS(n, a, b, solution, cost, infinite, best_objective, best_time, total_time);
